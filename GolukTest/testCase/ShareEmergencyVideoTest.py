...
class ShareEmergencyVideoTest(unittest.TestCase):
...
#         logging.basicConfig(filename=self.logPath+self.LOG_FILENAME,filemode='a+',level=logging.INFO,format = '%(asctime)s : %(message)s')
    ...
    def testShareEmergencyVideo(self):
        try:
            self.starttime=time.time()
            self.starttime2=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.starttime))
            
            self.l("==测试准备中==")
            self.filterlist=["btn 滤镜 古典胶片","btn 滤镜 黑白经典","btn 滤镜 柔和静谧","无","btn 滤镜 复古怀旧","btn 滤镜 多彩夏日","btn 滤镜 缤纷梦幻","btn 滤镜 清新淡雅"]
            ...
            if self.fvll>0:
                while n<self.fvll:
                    ...
                    if self.driver.find_elements_by_name("即刻分享")>0:
                        ...
                        self.l("分享选项: "+self.itemshare+" 已被选择")
