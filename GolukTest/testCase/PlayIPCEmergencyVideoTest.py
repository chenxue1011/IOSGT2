...
class PlayIPCEmergencyVideoTest(unittest.TestCase):
...
#         logging.basicConfig(filename=self.logPath+self.LOG_FILENAME,filemode='a+',level=logging.INFO,format = '%(asctime)s : %(message)s')
    ...
    def testIPCEmergencyVideo(self):
        try:
                    ...
                    while n<(self.fvll-1):
                        ...
                        if len(self.driver.find_elements_by_name("知道了"))>0:
                            ...
                            if len(self.driver.find_elements_by_name("我的相册")) == 0:
                                ...
                                raise NameError("视频播放超时，超时时间15秒")
                else:
                    self.l("Error: IPC远程没有紧急视频")
                    raise NameError('IPC远程没有紧急视频')         
            ...
            self.endtime=time.time()
            self.runtime=self.endtime-self.starttime
            self.l("RunTime: "+str(self.runtime))
