...
class VideoSquareTest(unittest.TestCase):
...
#         logging.basicConfig(filename=self.logPath+self.LOG_FILENAME,filemode='a+',level=logging.INFO,format = '%(asctime)s : %(message)s')
    ...
    def testVideoSquare(self):
        try:
            ...
            sleep(2)
            self.driver.find_element_by_name("广场").click()
            self.driver.find_element_by_name("img 曝光台").click()
#             for i in range(8):
